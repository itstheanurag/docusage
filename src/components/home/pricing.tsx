...
export type Plan = {
  ...
  features: string[];
  button?: PlanButton;
  /** visually highlight a plan (e.g., Pro) */
  highlight?: boolean;
  /** optional decorative icon / background element */
...
const defaultPlans: Plan[] = [
  ...
  },
  {
    ...
    highlight: true,
    accentIcon: <SparklesIcon className="w-32 h-32 text-white opacity-10" />,
  },
  {
    id: "team",
    name: "Team",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3">
          {items.map((plan, idx) => {
            ...
            const borderRight =
              idx < items.length - 1 ? "border-r border-neutral-200 dark:border-neutral-800" : "";
            const borderBottom = "border-b md:border-b-0";

            const cardBg =
              isHighlight
                ? "bg-neutral-900 dark:bg-black text-white relative overflow-hidden"
                : "bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100";

            return (
              <div
                key={plan.id ?? idx}
                className={`${baseCardClasses} ${borderBottom} ${borderRight} ${cardBg}`}
              >
                ...
                <div>
                  <h3 className={`text-xl font-bold mb-2 ${isHighlight ? "" : ""}`}>
                    {plan.name}
                  </h3>
                    ...
                    {plan.period && (
                      <span
                        className={`text-lg font-normal ${
                          isHighlight ? "text-neutral-400 dark:text-neutral-600" : "text-neutral-500 dark:text-neutral-400"
                        }`}
                      >
                        {plan.period}
                      </span>
                  ...
                  <ul className={`space-y-4 mb-8 ${isHighlight ? "text-neutral-300" : ""}`}>
                    {plan.features.map((f, i) => (
                      ...
                      >
                        <CheckCircleIcon className={`w-5 h-5 ${isHighlight ? "text-white" : ""}`} />
                        {f}
                      </li>
                    ))}
                  </ul>
